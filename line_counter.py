...
class LineCounter:
    ...
    def update(self, detections: Detections, vh_detections: Detections, frame: np.array, stream_name: str, folder_name: str, vehicle_classes: dict, Flip: bool):
        """
        ...
        for xyxy, confidence, class_id, tracker_id in detections:
            ...
            self.tracker_state[tracker_id] = tracker_state
            if tracker_state:
                for v_xyxy, v_confidence, v_class_id, v_tracker_id in vh_detections:
                    vx1, vy1, vx2, vy2 = v_xyxy
                    if v_tracker_id == tracker_id:
                        license_plate_crop = frame[int(y1):int(y2), int(x1):int(x2), :]
                        vehicle_crop = frame[int(vy1):int(vy2), int(vx1):int(vx2), :]
                        uid = f'{tracker_id}{random.randrange(1,999, 1)}'
                        date = datetime.now().strftime('%Y-%m-%d')
                        time = datetime.now().strftime('%H:%M:%S')
                        cv2.imwrite(f'./temp/{folder_name}/{uid}.jpg', license_plate_crop)
                        cv2.imwrite(f'./cache/{folder_name}/{uid}.jpg', vehicle_crop)
                        write_temp(uid, stream_name.upper(), date, time, vehicle_classes[v_class_id].upper(), 'EXIT', folder_name)
                        self.in_count += 1
            else:
                for v_xyxy, v_confidence, v_class_id, v_tracker_id in vh_detections:
                    vx1, vy1, vx2, vy2 = v_xyxy
                    if v_tracker_id == tracker_id:
                        ...
                        cv2.imwrite(f'./temp/{folder_name}/{uid}.jpg', license_plate_crop)
                        cv2.imwrite(f'./cache/{folder_name}/{uid}.jpg', vehicle_crop)
                        write_temp(uid, stream_name.upper(), date, time, vehicle_classes[v_class_id].upper(), 'ENTRY', folder_name)
                        self.out_count += 1
        if Flip:
...
class LineCounterAnnotator:
    ...
    def annotate(self, frame: np.ndarray, line_counter: LineCounter, Flip: bool) -> np.ndarray:
        """
        ...
        in_text = f"{text1}: {line_counter.in_count}"
        out_text = f"{text2}: {line_counter.out_count}"
